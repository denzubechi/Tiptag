...
export async function POST(request: NextRequest) {
  try {
    ...
    if (existingUser) {
      return NextResponse.json({ error: "User already exists with this email or wallet" }, { status: 400 })
    }
    ...
    if (existingTipTag) {
      return NextResponse.json({ error: "Tip tag is already taken" }, { status: 400 })
    }
